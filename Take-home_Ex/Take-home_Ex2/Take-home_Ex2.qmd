...
9.  Lastly, given our recent lesson on interactivity, we can also apply tooltips to the bars for easy comprehension. A simple example is given below produced by `ggiraph`. The tooltip presents the sex, age group, percentage and population. (*Figure adjustments using special Quarto syntax seem to not affect this figure so work needs to be done to adjust the plot.*)
